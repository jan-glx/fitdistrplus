      ...
      }else #weighted version from util-wtdstat.R
      {
       ...
       if (distname == "geom" ) {
        ...
        if (distname == "logis" ) {
		    ...
		    loglikval <- tryCatch(loglik(estimate, fix.arg, data, ddistname), error = function(e) NULL)
    ...
    }else #an optimimisation has to be done, where fix.arg and start can be a function
    {
        ...
        if(is.null(custom.optim))
        {
            ...
            res <- list(estimate = opt$par, convergence = opt$convergence, value = opt$value,
                    ...
                    loglik=tryCatch(loglik(opt$par, fix.arg, data, ddistname), error = function(e) NULL),
